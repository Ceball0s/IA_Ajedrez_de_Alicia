...
class tablero_magico:
    ...
    def generar_movimientos(self):
        # Generar movimientos legales en el tablero 1 para las piezas del turno actual
        lista_movimiento = [
            move for move in self.tablero1.legal_moves
            if not self.casilla_ocupada(self.tablero2, move.to_square) and
            self.tablero1.piece_at(move.from_square).color == self.color
        ]

        # Generar movimientos legales en el tablero 2 para las piezas del turno actual
        lista_movimiento += [
            move for move in self.tablero2.legal_moves
            if not self.casilla_ocupada(self.tablero1, move.to_square) and
            self.tablero2.piece_at(move.from_square).color == self.color
        ]
    ...
    def move(self, origen, destino):
        ...
        if piece_tablero1:
            if self.verificar_movimiento(origen, destino, self.tablero1, self.tablero2):
                ...
                self.tablero1.set_piece_at(origen, None)
                self.tablero1.set_piece_at(destino, None)
            ...
            if self.verificar_movimiento(origen, destino, self.tablero2, self.tablero1):
                self.tablero1_antes = self.tablero1.copy()
                self.tablero2_antes = self.tablero2.copy()
                self.tablero2.set_piece_at(origen, None)
                self.tablero2.set_piece_at(destino, None)
    ...
    def pop(self):
        self.tablero1 = self.tablero1_antes
        self.tablero2 = self.tablero2_antes
    ...
    def promover_peones(self):
        ...
        for square in range(64):
            ...
            if pieza and pieza.piece_type == chess.PAWN:
                ...
                (pieza.color == chess.BLACK and chess.square_rank(square) == 0):
                    # Promover el peón a reina
                    self.tablero2.set_pievaluece_at(square, chess.Piece(chess.QUEEN, pieza.color))
